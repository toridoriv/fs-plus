 ...
 */
export function isDirectory(entry: fs.Dirent) {
 ...
 */
export function isFile(entry: fs.Dirent) {
  return entry.isFile();
}
 ...
 */
export function toPath(entry: fs.Dirent) {
  return np.join(entry.parentPath, entry.name);
}

/**
 * Represents a function that receives a path and returns whether to include the path in
 * the results.
 *
 ...
 * @returns `true`.
 */
export function includePath(_path: string) {
  return true;
}
 ...
 */
export function readDirectoryFiles(path: string, filter: FilterPath = includePath) {
 ...
 */
export function readDirectorySubdirectories(
  path: string,
  filter: FilterPath = includePath,
) {
